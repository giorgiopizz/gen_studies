...
def fit_1d(fout, op1, m1, npoints, **kwargs):
    ...
    for coeff in c1:
        _init_pars = init_pars.copy()
        res.append(fit1d(op1, coeff, params, _init_pars, **kwargs))
...
for op in tot_ops:
    ...
    for variable in variables:
        ...
        workspace = read_ws(f"cards/{variable}.json", ops)
        # fit_1d('cHWtil', m1=10, npoints=100)

        if len(ops) == 1:
            fit_1d(fout, *ops, *limits, npoints=npoints, **workspace)
        else:
            fit_2d(fout, *ops, *limits, npoints=npoints, **workspace)
